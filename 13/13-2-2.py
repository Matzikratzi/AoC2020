...
startN = 357732110 # This is a good starting value. Trial and error!
n = startN
while True:
    ...
    if (    (t + indList[1]) % idList[1] == 0
        ...
        and (t + indList[5]) % idList[5] == 0):
        print('tested time', t, n)

        if ((t + indList[6]) % idList[6] == 0
            and (t + indList[7]) % idList[7] == 0
            and (t + indList[8]) % idList[8] == 0):
            print('solution:',t)
            break
        n += 413842151 # Found out after experiments
    else:
        n += 1
